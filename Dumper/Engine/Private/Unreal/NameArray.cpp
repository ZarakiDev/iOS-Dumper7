...
inline std::pair<uintptr_t, bool> FindFNameGetNamesOrGNames(uintptr_t EnterCriticalSectionAddress, uintptr_t StartAddress)
{
	...
	const uint8* BytePropertyStringAddress = static_cast<uint8*>(FindByStringInAllSections(TEXT("ByteProperty"), StartAddress));
...
bool NameArray::TryFindNamePool()
{
	...
	while (!NamePoolIntance)
	{
		...
		for (int i = 0; i < InitSRWLockSearchRange; i++)
		{
			...
			MemAddress StringRef = FindByStringInAllSections(TEXT("ByteProperty"), PossibleConstructorAddress, BytePropertySearchRange);
